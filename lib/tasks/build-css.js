...
function processCss() {
  ...
  return gulp.src(config.roots.css).pipe(print(function (filepath) {
    return 'css: ' + filepath;
  })).pipe(plumber(utils.errorHandler)).pipe(scssFilter).pipe(sourcemaps.init()).pipe(sass({ outputStyle: 'compressed' }).on('error', sass.logError)).pipe(sourcemaps.write('.')).pipe(scssFilter.restore).pipe(cssimport()).pipe(gulp.dest(config.dist.assets));
}
