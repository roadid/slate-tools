...
function processThemeJs() {
  ...
  return gulp.src([config.roots.js, '!' + config.roots.vendorJs]).pipe(sourcemaps.init()).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(ngAnnotate()).pipe(uglify({
    mangle: true,
    compress: true,
    preserveComments: 'license'
  })).pipe(sourcemaps.write('.')).pipe(gulp.dest(config.dist.assets));
}
