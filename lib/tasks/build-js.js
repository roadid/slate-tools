...
function processVendorJs() {
  ...
  return merge2(gulp.src(config.roots.vendorJs), gulp.src(mainBowerFiles()).pipe(jsFilter)).pipe(sourcemaps.init()).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(uglify({
    mangle: true,
    compress: true,
