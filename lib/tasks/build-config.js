...
function processConfig(file) {
  ...
  return gulp.src(file).pipe(plumber(utils.errorHandler)
  // .pipe(size({
  //   showFiles: true,
  //   pretty: true,
  // }))
...
gulp.task('watch:config', function () {
  chokidar.watch(config.tkConfig, { ignoreInitial: true }).on('all', function (event, path) {
...
gulp.task('watch:dist-config', function () {
  chokidar.watch(config.dist.root + config.tkConfig, { ignoreInitial: true }).on('all', function (event, path) {
    messages.logFileEvent(event, path);
