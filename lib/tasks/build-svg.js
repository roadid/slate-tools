...
function processIcons(files) {
  ...
  return gulp.src(files).pipe(plumber(utils.errorHandler)).pipe(svgmin(config.plugins.svgmin)).pipe(cheerio(config.plugins.cheerio)).pipe(extReplace('.liquid')).pipe(gulp.dest(config.dist.snippets));
}
...
function removeIcons(files) {
  ...
  return gulp.src(mapFiles).pipe(plumber(utils.errorHandler)).pipe(vinylPaths(del));
}
...
gulp.task('watch:svg', function () {
  ...
  chokidar.watch([config.src.icons], { ignoreInitial: true }).on('all', function (event, path) {
