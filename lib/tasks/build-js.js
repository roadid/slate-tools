...
function processVendorJs() {
  ...
  return gulp.src(config.roots.vendorJs).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(print()).pipe(uglify({
    mangle: true,
    compress: true,
...
function processBowerVendorJs() {
  ...
  return gulp.src(mainBowerFiles()).pipe(jsFilter).pipe(plumber(utils.errorHandler)).pipe(include()).pipe(print()).pipe(uglify({
    mangle: true,
    compress: true,
    preserveComments: 'license'
  })).pipe(flatten()).pipe(gulp.dest(config.dist.assets));
}
