...
function processCss() {
  ...
  return gulp.src(config.roots.css).pipe(plumber(utils.errorHandler)).pipe(scssFilter).pipe(sourcemaps.init()).pipe(sassGlob()).pipe(sass({ outputStyle: 'compressed' })).pipe(sourcemaps.write('.')).pipe(scssFilter.restore).pipe(gulp.dest(config.dist.assets));
}
