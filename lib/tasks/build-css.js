...
function processCss() {
  ...
  return gulp.src(config.roots.css).pipe(plumber(utils.errorHandler)).pipe(scssFilter).pipe(sass().on('error', sass.logError)).pipe(scssFilter.restore).pipe(cssimport()).pipe(extReplace('.css.liquid', '.css')).pipe(extReplace('.scss.liquid', '.scss')).pipe(gulp.dest(config.dist.assets));
}
